...
| D6 | Rumble Pak and Controller Pak | both on one slot (BAR, deliberate hardware deviation) / Pak only, no rumble (RAY2) / pak swap prompt as on hardware | **both on one slot**: saves and rumble with no swap. Needs a pak answer that reads as a Rumble Pak and a one-instruction game patch to the slot classifier (phase 05) | recommended option, autonomous session (2026-09-15) |
