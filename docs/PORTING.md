...
**Status:** phase 04 done: boots through logos, title, menus, pak prompts and the opening cinematic into exploration, controllable with the stick; no lookup miss in 15 minutes. Phases 05 (saves, rumble, settings), 06 (frontend) and 07 (widescreen in several aspect ratios) confirmed by Daniel's playtest; 0.1.0 released for Windows and Linux; 0.2.0 fixes the crash at the first enemy encounter (issue 001), played by Daniel through five battles. Phase 08 (high frame rate) next. Sections below are filled as phases land.
