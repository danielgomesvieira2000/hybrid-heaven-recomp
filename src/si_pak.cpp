...
bool trace_enabled() {
    static const bool on = std::getenv("HH_PAKTRACE") != nullptr;
    return on;
}
...
void handle_pak_block(uint8_t* rdram, int64_t base, int port, unsigned cmd) {
    ...
    uint8_t data[hh::pak::kBlockSize];
    if (is_write) {
    ...
    if (block == kBlockDetect) {
        ...
        if (is_write) {
            hh::pak::write_block(port, block, data);
        }
        else {
    ...
    if (trace_enabled()) {
        std::fprintf(stderr, "[hh] pak %s port=%d block=0x%03X data=%02X%02X%02X%02X\n",
...
void handle_status(uint8_t* rdram, int64_t base, int port) {
    const bool has_pak = hh::pak::present(port);
    ...
    if (trace_enabled()) {
        std::fprintf(stderr, "[hh] pak status port=%d -> %s\n",
...
extern "C" void __osSiRawStartDma_recomp(uint8_t* rdram, recomp_context* ctx) {
    ...
    if (direction == 0 && usable) {
        if (!service_frame(rdram, pif) && trace_enabled()) {
            std::fprintf(stderr, "[hh] pak: unrecognised SI frame:");
            for (int i = 0; i < 12; i++) {
