...
void ensure_loaded(int port) {
    ...
    if (!g_dir.empty()) {
        ...
        if (fs::exists(pak_path(port), ec)) {
            ...
            if (in) {
                g_pak[port].assign(hh::pak::kPakSize, 0);
                in.read(reinterpret_cast<char*>(g_pak[port].data()), hh::pak::kPakSize);
                // A short file is a truncated write from a previous run, not a
                // pak. Formatting over it loses nothing that was readable.
                read_from_disk = (in.gcount() == hh::pak::kPakSize);
            }
        }
    }
    ...
    if (!read_from_disk) {
        ...
        if (!g_dir.empty()) {
            ...
            if (out) {
                out.write(reinterpret_cast<const char*>(g_pak[port].data()), hh::pak::kPakSize);
                std::fprintf(stderr, "[hh] created Controller Pak \"%s\"\n",
