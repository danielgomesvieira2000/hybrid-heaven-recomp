// The HUD inspector. See include/hh/inspector.h for what it is for.
//
// Copied from Wave Race 64: Recompiled (src/inspector.cpp) without its water sun
// editor. The panel and the override table are the port-independent half; the
// classifier that fills them is written for this game in phase 07.
...
void draw_panel() {
    ...
    if (!ImGui::Begin("Hybrid Heaven HUD")) {
        ImGui::End();
        return;
    }
...
// Every window the port puts in RT64's F1 menu. RT64 has one hook, so this is
// where any later window is added.
void draw_windows() {
    draw_panel();
}
...
void init() {
    ...
    // until RT64's UI is open. HH_INSPECTOR=0 turns the port's half off for
    // an A/B; RT64's own debug menu stays on F1 either way.
    const char* env = std::getenv("HH_INSPECTOR");
...
// Test hook: HH_TEST_HUD_OVERRIDE=<identity>=<class>@<seconds> sets an override
...
// spill.
void test_override() {
    static const char* spec = std::getenv("HH_TEST_HUD_OVERRIDE");
    ...
    for (int c = 0; c < 5; ++c) {
        if (name == names[c]) {
            ...
            g_any_overrides.store(true, std::memory_order_relaxed);
            std::fprintf(stderr, "[hh] test: HUD override %s -> %s\n", identity.c_str(), name.c_str());
            std::fflush(stderr);
        }
    }
    done = true;
