...
RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle,
    ...
    app_config.appId = "hybrid-heaven-recomp";
...
void RT64Context::send_dl(const OSTask* task) {
    ...
    //
    // Bisect switch for phase 04: setting HH_SKIP_DL isolates whether a fault
    // is inside RT64's display list processing or somewhere else entirely.
    static const bool skip_dl = std::getenv("HH_SKIP_DL") != nullptr;
    if (!skip_dl) {
        app_->processDisplayLists(app_->core.RDRAM, physical(task->t.data_ptr), 0, true);
    }

    if (dl_count == 1) {
        std::fprintf(stderr, "[hh] send_dl returned from the first display list\n");
        std::fflush(stderr);
    }
}
