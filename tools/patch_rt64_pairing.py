...
RT64_GetTransformPairing and prints rates under HH_PAIRING=1.
...
EDITS = [
    (
        ...
        '#include "xxHash/xxh3.h"\n'
        "\n"
        "// hh: for the transform-pairing counters below.\n"
        "#include <unordered_set>\n",
    ),
    (
        "namespace RT64 {\n    // GameFrame\n",
        "// hh: running totals of the transform pairing, read by the port through the\n"
        ...
        "static uint64_t hhPairingFrames = 0;\n"
        "static uint64_t hhPairingTotal = 0;\n"
        "static uint64_t hhPairingIgnored = 0;\n"
        "static uint64_t hhPairingUnpaired = 0;\n"
        "static uint64_t hhPairingUnpairedMoved = 0;\n"
        "\n"
        ...
        "    *frames = hhPairingFrames;\n"
        "    *total = hhPairingTotal;\n"
        "    *ignored = hhPairingIgnored;\n"
        "    *unpaired = hhPairingUnpaired;\n"
        "    *unpairedMoved = hhPairingUnpairedMoved;\n"
        "}\n"
        "\n"
        "namespace RT64 {\n    // GameFrame\n",
    ),
    (
        ...
        "        matchScenes(orthographicScenes, prevFrame.orthographicScenes);\n"
        "\n"
        "        // hh: count what failed to pair (tools/patch_rt64_pairing.py).\n"
        "        {\n"
        "            thread_local std::unordered_set<uint64_t> hhPrevMatrices;\n"
        "            hhPrevMatrices.clear();\n"
        ...
        "                    hhPrevMatrices.insert(XXH3_64bits(&m, sizeof(m)));\n"
        "                }\n"
        "            }\n"
        "\n"
        ...
        "                    if (hhPrevMatrices.find(XXH3_64bits(&m, sizeof(m))) == hhPrevMatrices.end()) {\n"
        "                        unpairedMoved++;\n"
        "                    }\n"
        "                }\n"
        "            }\n"
        "\n"
        "            hhPairingFrames++;\n"
        "            hhPairingTotal += total;\n"
        "            hhPairingIgnored += ignored;\n"
        "            hhPairingUnpaired += unpaired;\n"
        "            hhPairingUnpairedMoved += unpairedMoved;\n"
        "        }\n",
    ),
]
