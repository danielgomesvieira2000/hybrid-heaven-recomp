...
"""Launch the port and record the frames its window presents, even when covered.
    ...
    python tools/capture_frames.py OUTDIR FROM TO --exe PATH --title "WINDOW TITLE"
                                   [--scale 0.5] [--rom PATH] [--env NAME=VALUE ...]
...
def main():
    ...
    ap.add_argument("--exe", required=True, help="the port's executable")
    ap.add_argument("--title", required=True, help="the port's exact window title")
    ...
    while time.perf_counter() - launched < 60 and proc.poll() is None:
        hwnd = window_of_process(proc.pid, args.title)
        if hwnd:
            break
        time.sleep(0.2)
    if not hwnd:
        proc.kill()
        sys.exit("no window titled '%s' appeared for process %d" % (args.title, proc.pid))
