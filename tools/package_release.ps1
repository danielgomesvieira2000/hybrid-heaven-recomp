...
$exe = Join-Path $BuildDir "hybrid-heaven-recomp.exe"
if (-not (Test-Path $exe)) {
    throw "no executable at $exe -- build first (see docs/BUILDING.md)"
}

$name = "hybrid-heaven-recomp-$Version-windows-x64"
$stage = Join-Path $OutDir $name
if (Test-Path $stage) { Remove-Item -Recurse -Force $stage }
New-Item -ItemType Directory -Force $stage | Out-Null

$files = @(
    "$BuildDir\hybrid-heaven-recomp.exe",
    "$BuildDir\SDL2.dll",
...
$forbidden = Get-ChildItem -Recurse $stage | Where-Object {
    $_.Extension -in ".z64", ".n64", ".v64", ".rom", ".bin", ".eep", ".sra", ".fla", ".pak", ".mpk", ".srm"
}
if ($forbidden) {
...
$pdb = Join-Path $BuildDir "hybrid-heaven-recomp.pdb"
if (Test-Path $pdb) {
