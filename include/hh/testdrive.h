...
// where it got to.
//
// Whether the menus work and the game runs is not observable from outside the
// process: a port that hangs on the title screen and one that is quietly playing
...
//
// A watcher on the game's state turns the run into a transcript. For Hybrid
// Heaven the state variable is not identified yet; the file-load log
// (HH_DEBUG_LOADS) stands in for it.

namespace hh {

// Loads a script of timed inputs from the path in HH_INPUT_SCRIPT, if set.
...
// Reports changes of the game state (none yet: see src/testdrive.cpp). Called once per frame from the
